...
def plot_file(filename, index=0, timestamps=True, children=True, legend=None, options=None):
    try:
        import pylab as pl
    except ImportError as e:
        print("matplotlib is needed for plotting.")
...
such file in the current directory."""
    ...
    parser.add_argument('--version', action='version', version=mp.__version__)
    parser.add_argument('--legends', "-l", action='append', type=str)
    ...
    fig = pl.figure(figsize=(11, 6), dpi=90)
    ...
    for n, filename in enumerate(filenames):
        if legends:
            ...
            legend = None
        mprofile = plotter(filename, index=n, timestamps=timestamps, legend=legend, options=args)
    pl.xlabel("Tempo (em segundos)", fontsize=16)
    pl.ylabel("Memória (em MB)", fontsize=16)

    if args.title is None and len(filenames) == 1:
        pl.title(mprofile['cmd_line'])
    else:
        if args.title is not None:
            pl.title(args.title, fontsize=16)
