...
class My_cnn(object):
    ...
    def model1(self):
        ...
        for train, test in kfold.split(self.all_x, self.all_y):
            print("MODEL1: RUNNING FOLD ", fold)
            filters = [2, 3]
            model_input = Input(shape=self.input_shape)
            conv_blocks = []
            for block_size in filters:
                conv = Conv1D(filters=128, kernel_size=block_size, padding='valid', activation='relu', strides=1)(model_input)
                conv = Dropout(0.2)(conv)
                conv = MaxPooling1D(pool_size=2)(conv)
                conv = Conv1D(filters=64, kernel_size=block_size, padding='valid', activation='relu', strides=1)(conv)
                conv = Dropout(0.2)(conv)
                conv = MaxPooling1D(pool_size=2)(conv)
                conv = Flatten()(conv)
                ...
                conv1 = Conv1D(filters=32, kernel_size=block_size, padding='valid', activation='relu', strides=1)(conv1)
                conv1 = Dropout(0.2)(conv1)
                conv1 = MaxPooling1D(pool_size=2)(conv1)
                conv1 = Flatten()(conv1)
            ...
            model.fit(data_train, keras.utils.to_categorical(self.all_y[train], self.num_classes), batch_size=32, epochs=30, verbose=2, validation_data=(data_test, keras.utils.to_categorical(self.all_y[test], self.num_classes)))
